...
if (F){
  cost_matrix_test <- read.csv("/data/rihao/project_with_xiuyuan/pbmc_10k_v3/result/data/MM_dm5_graph_N1000_v2/ot_cost.csv", header = F)
  dim(cost_matrix_test)
  cost_matrix_test[1:5, 1:5]
  cost_matrix[1:5, 1:5]
...
################################################
##### EMD computation in Python
################################################
#####Save the files for EMD computation in Python
N <- 1000
dir.path <- paste0("/data/rihao/project_with_xiuyuan/pbmc_10k_v3/result/data/", "MM_dm5_graph_N", N, "_v2/")
if (T){
  dir.create(dir.path)
...
################################################
##### Visualizing gene trajectory
################################################
gene.dist.mat <- read_gene.dist.mat(dir.path, file_name = "emd.csv")
str(gene.dist.mat)

gene_embedding <- GetGeneEmbedding(gene.dist.mat, K = 5)$diffu.emb


gene_trajectory <- identify_trajectories_v2(gene_embedding, gene.dist.mat, N = 3, t = c(11,21,8), quantile = 0.02, K=5)#,7.75))
...
data_S <- add_gene_set_score(data_S, gene_trajectory, N_bin = 5, trajectories = 1:3, binarize = T, assay = "alra", echo = T, reverse = c(F, F, T))
